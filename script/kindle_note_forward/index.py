...
def export_note(kindle_note_path: str):
    ...
    for file_name in os.listdir(origin_html_path):
        ...
        for note_heading in note_headings:
            ...
            # 获取书摘内容
            note_content = note_heading.find_next_sibling('div', class_='noteText').text.strip()
            # 获取书摘位置
            note_position = note_heading_text[note_heading_text.find("位置 ") + 3:]
            # 获取章节名
            note_chapter = note_heading_text[note_heading_text.find(" - ") + 3:note_heading_text.find(" > 位置")]
            # 获取书摘的笔记
            next_note_heading = note_heading.find_next_sibling('div', class_='noteHeading')
            comments = ""
            if next_note_heading.text.find("笔记") != -1:
                comments_div = next_note_heading.find_next_sibling('div', class_='noteText')
...
def parse_cmd_args(args):
    ...
    parser.add_argument('--markdown_path', type=str, default='kindle_note', help='markdown path')
    parser.add_argument('--html_path', type=str, default='kindle_note/origin', help='html path')
    parser.add_argument('--atlas_uri', type=str, default='mongodb://localhost:27017/', help='mongodb atlas uri')
