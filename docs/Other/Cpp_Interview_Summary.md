...
Map是一种映射关系，也就是说它存储的是键值对(key-value pair)。Map中的每一个元素都是一个键值对，其中键是唯一的，值可以是任意类型。

Set是一种集合，它存储的是一组无序的，不重复的数据。 Set中的每一个元素都是唯一的，并且可以是任意类型。

Map可以使用红黑树或哈希表来实现，Set可以使用红黑树或平衡树来实现。
...
使用allocator的一个主要优点是，可以使用不同的内存分配器来替换默认的内存分配器。这样可以提高程序的内存使用效率，或者为了满足特殊的内存分配需求。

:::

::: tip 请你来说一说STL迭代器删除元素

对于序列容器vector,deque来说,由于连续分配内存 , 会导致后续元素前移 , 因此后续迭代器全部失效 . 所以`erase`方法返回下一个有效迭代器 . 关联类容器`map`和`set`底层实现是红黑树,不影响迭代器,直接迭代器自增可得到下一个有效迭代器 . `list`是不连续分配的 , 因此`erase`方法也返回下一个有效迭代器.
