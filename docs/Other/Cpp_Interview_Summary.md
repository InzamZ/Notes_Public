...
在 C++中，对与非内建型别来说，前自增运算 (++i) 返回的是对象的引用，而后自增运算返回的是对象，返回对象将造成拷贝构造函数更多的调用，所以++i 的效率要高与 i++。但是，对于内建型别 (也就是 int,short,char 等类型)，i++与++i 的效率几乎没有区别 (因为内建型别没有拷贝构造函数)。
...
::: tip 请你回答一下new/delete与malloc/free的区别是什么

1. 语言的不同：`new` 和 `delete` 是 C++ 中的运算符，而 `malloc` 和 `free` 是 C 语言中的函数。
2. 分配的内存类型不同：`new` 运算符会分配被初始化的内存，而 `malloc` 函数会分配未初始化的内存。
3. 返回值的类型不同：`new` 运算符会返回分配的内存的指针，而 `malloc` 函数会返回一个 void 类型的指针。
4. 异常处理的不同：`new` 运算符会抛出异常 (`std::bad_alloc`)，当无法分配内存时；而 `malloc` 函数会返回空指针 (`NULL`)。

:::

::: tip 请你说说fork,wait,exec函数

`fork` 函数是操作系统函数，用于在当前进程中创建一个新的子进程。调用 `fork` 函数时，会创建一个新的进程，新进程的进程号为函数的返回值。在新进程中，函数的返回值为 0，而在原进程中，函数的返回值为新进程的进程号。

`wait` 函数是操作系统函数，用于在父进程中等待子进程的结束。`wait` 函数会阻塞当前进程，直到有一个子进程结束。如果传入的参数不为空，则会将子进程的终止状态存储到参数中。函数的返回值为结束的子进程的进程号。`waitpid` 函数类似于 `wait` 函数，但可以指定等待的子进程的进程号，以及选项。

`exec` 函数是操作系统函数，用于在当前进程中执行新的程序。在调用 `exec` 函数族中的任何一个函数时，都会在当前进程的地址空间中加载新的程序，并用新的程序替换当前进程的代码。如果调用成功，则新的程序会在原来进程的地址空间中运行，原来的进程代码被完全替换。如果调用失败，则会返回错误码。
...
如果同时定义了两个函数，一个带 const 修饰，一个不带，则编译器会视它们为两个不同的函数，这是没有问题的。这种编译器会自动决定使用那个函数，对常量才会使用 const 版本的函数。

:::

::: tip 请你来说一说C++函数栈空间的最大值

可以使用 `ulimit` 命令或 `getrlimit` 函数来查看函数栈空间的大小限制。你也可以在程序启动时使用 `RLIMIT_STACK` 环境变量来设置函数栈空间的大小。你还可以使用 `pthread_attr_setstacksize` 函数来设置线程的函数栈空间的大小。

:::

::: tip 请你来说一说extern“C”

`extern "C"` 修饰符是 C++ 中用于调用 C 语言代码的一种有效方法，可以让你在 C++ 程序中方便地使用 C 语言函数、变量和函数指针。

:::

::: tip 请说一下你了解的RTTI

RTTI (运行时类型信息) 是 C++ 中的一种机制，它允许在程序运行时检查对象的类型。在 C++ 中，你可以使用 `typeid` 运算符获取对象的类型信息。你也可以使用 `dynamic_cast` 运算符在运行时将对象的指针转换为其他类型的指针。

:::

::: tip 请你说说虚函数表具体是怎样实现运行时多态的?

虚函数表 (virtual function table，简称 vtable) 是 C++ 中的一种机制，用于实现运行时多态。

当一个类定义了虚函数 (virtual function) 时，编译器会为该类创建一个虚函数表。虚函数表是一个数组，数组中的每一项都是一个函数指针，指向类中的虚函数。当你在程序中创建了这个类的对象时，编译器会在对象的内存布局中添加一个指针，指向这个对象的虚函数表。这个指针称为虚函数指针 (virtual function pointer)。当你在程序中调用一个虚函数时，编译器会检查这个函数的调用者是否有虚函数指针。如果有，则编译器会使用这个指针来访问虚函数表，从而找到这个函数的实际实现。

:::

::: tip 说说C语言是怎么进行函数调用的？以及参数压栈顺序？

当函数被调用时，系统会在内存中为函数开辟一个新的工作区域，用来存储函数的局部变量和临时数据。这个工作区域通常被称为函数的调用栈，在大多数系统中都是通过压栈来实现的。

在函数调用期间，会执行以下操作来压入函数的调用栈：

1. 将函数的参数压入堆栈。在调用函数时，会将函数的参数从右到左依次压入堆栈，以便在函数内部使用。
2. 将返回地址压入堆栈。当函数执行完毕后，系统需要知道应该回到哪里继续执行。因此会将调用函数的语句后面的地址压入堆栈，以便在函数返回时使用。
3. 讲调用函数的 ebp 指针压栈保存，在函数运行的时候 ebp 会替换成调用栈的 ebp，寄存器的值需要后续恢复。
4. 将函数的局部变量和临时数据压入堆栈。函数内部可能会定义局部变量或使用临时数据，这些数据都需要在函数调用帧中保存。
5. 跳转到函数的入口地址执行函数体。
...
对于函数返回值类型为非引用类型，都是会在内存中创建一个临时变量，将返回值拷贝到临时变量中，而返回值是作为函数调用栈区中的局部变量，随着函数的返回，栈区的销毁，而被销毁。

:::

::: tip 回答一下C++中拷贝赋值函数的形参能否进行值传递？

**可以。但是调用拷贝赋值函数的时候，首先要将实参传递给形参，此时会多调用一次拷贝构造，并不会无限递归。**

参考 blog：https://blog.csdn.net/qq_34008220/article/details/116561645

:::

::: tip 请你说一说select

`select` 是一种网络编程函数，可以在多个网络描述符上进行 I/O 多路复用。

`select` 函数的原型如下：

```cpp
Copy codeint select(int nfds, fd_set* readfds, fd_set* writefds,
...
- `nfds`：指定监听的最大描述符值加 1。
- `readfds`：指向一个描述符集，表示需要监听的可读描述符。
- `writefds`：指向一个描述符集，表示需要监听的可写描述符。
- `exceptfds`：指向一个描述符集，表示需要监听的异常描述符。
- `timeout`：指向一个时间值结构体，表示超时时间。

当 `select` 函数调用成功时，会返回就绪的文件描述符数目，并将就绪的文件描述符集合存储到对应的文件描述符集合参数中。如果在超时时间内没有文件描述符就绪，则会返回 0。

:::

## 容器和算法

::: tip 说一下map和set有什么区别，分别又是怎么实现的?

Map 是一种映射关系，也就是说它存储的是键值对 (key-value pair)。Map 中的每一个元素都是一个键值对，其中键是唯一的，值可以是任意类型。

Set 是一种集合，它存储的是一组无序的，不重复的数据。Set 中的每一个元素都是唯一的，并且可以是任意类型。

Map 可以使用红黑树或哈希表来实现，Set 可以使用红黑树或平衡树来实现。

:::

::: tip 介绍一下STL的allocator?

`STL allocator` 封装了 `STL` 容器在内存管理上的底层细节。在 `STL` 中，通过 `STL allocator` 将对象构造/析构和内存分配/释放两个步骤分开来。其中，对象的构造和析构是分别通过 `allocator::construct()` (使用 `placement new` 来调用对象的构造函数) 和 `allocator::destory()` (使用对象的析构函数) 来实现。内存分配/释放是分别通过 `allocator::allocate()` (通过 `malloc` 分配内存) 和 `allocator::deallocate()` (通过 `free` 释放内存) 来实现。

使用 allocator 的一个主要优点是，可以使用不同的内存分配器来替换默认的内存分配器。这样可以提高程序的内存使用效率，或者为了满足特殊的内存分配需求。

:::

::: tip 请你来说一说STL迭代器删除元素

对于序列容器 vector,deque 来说，由于连续分配内存，会导致后续元素前移，因此后续迭代器全部失效。所以 `erase` 方法返回下一个有效迭代器。关联类容器 `map` 和 `set` 底层实现是红黑树，不影响迭代器，直接迭代器自增可得到下一个有效迭代器。`list` 是不连续分配的，因此 `erase` 方法也返回下一个有效迭代器。

:::

:::请你讲讲STL有什么基本组成

STL 主要由以下几部分组成：容器，迭代器，仿函数，算法，分配器，配接器。
